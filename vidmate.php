...
.closa{user-select:none;cursor: pointer; font-weight: bolder; color: white; background-color: red; height: 25px; width: 25px; font-size: 20px; text-align: center; margin-top: 50px; font-family: arial;z-index: 1;float: right; position: absolute; top: -50; right: 0; }
#mode{ display: none;}
.sesl{width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); position: fixed; z-index: 999; top: 0; left: 0; padding-top: 0.5%;}
#vid{max-height: 100%; max-width: 100%;}
...
.dot1 span{float: left; background-color: white; border: 1px solid black; padding: 3px; margin: 5px 2px; margin-top: 10px; border-radius: 100px;}
...
var doctor = document.getElementsByName('video');
var object = document.getElementsByName('camo');
var k;
			for (k = 0; k < doctor.length; k++ ){
				doctor[k].onclick = function(){
			document.getElementById('vid').src = this.src;		
			mode.style.display = "block";
			var str = this.src;
			var loc = str.lastIndexOf("upload/");
			var slice = str.slice(7+loc);
			
			document.getElementById("downgrade").download = slice;
			document.getElementById("downgrade").href = this.src;
				}
			}
...
for (k = 0; k < object.length; k++ ){
			...
			document.getElementById("downgrade").download = slice;
			document.getElementById("downgrade").href = this.id;
				}
			}
		...
		function songi(){
			...
			document.getElementById("downgrade").download = src;
			document.getElementById("downgrade").href = src;
		}
